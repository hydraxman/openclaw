...
- `channel` optional (string): The messaging channel for delivery. One of: `last`, `telegram`, `discord`, `slack`, `mattermost` (plugin), `signal`, `msteams`. Defaults to `last`.
- `to` optional (string): The recipient identifier for the channel (e.g., phone number for Signal, chat ID for Telegram, channel ID for Discord/Slack/Mattermost (plugin), conversation ID for MS Teams). Defaults to the last recipient in the main session.
...
<<<<<<< HEAD
  (`channel` defaults to `last` and falls back to the default channel).
=======
