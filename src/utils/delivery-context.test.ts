...
describe("delivery context helpers", () => {
  ...
  it("builds stable keys only when channel and to are present", () => {
    expect(deliveryContextKey({ channel: "signal", to: "+1555" })).toBe("signal|+1555||");
    expect(deliveryContextKey({ channel: "signal" })).toBeUndefined();
    expect(deliveryContextKey({ channel: "signal", to: "+1555", accountId: "acct-1" })).toBe(
      "signal|+1555|acct-1|",
    );
  ...
  it("normalizes delivery fields and mirrors them on session entries", () => {
    ...
    expect(normalized.lastChannel).toBe("signal");
